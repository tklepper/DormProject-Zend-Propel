...
abstract class Unit implements ActiveRecordInterface
{
    ...
    /**
     * The value for the building_number field.
     *
     * @var        int
     */
    ...
    /**
     * Get the [building_number] column value.
     *
     * @return int
     */
    ...
    /**
     * Set the value of [building_number] column.
     *
     * @param int $v new value
     * @return $this|\Unit The current object (for fluent API support)
     */
    public function setBuildingNumber($v)
    {
        ...
        if ($this->building_number !== $v) {
            $this->building_number = $v;
            $this->modifiedColumns[UnitTableMap::COL_BUILDING_NUMBER] = true;
        }
    ...
    public function setRoomNumber($v)
    {
        ...
        if ($this->room_number !== $v) {
            $this->room_number = $v;
            $this->modifiedColumns[UnitTableMap::COL_ROOM_NUMBER] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : UnitTableMap::translateFieldName('BuildingNumber', TableMap::TYPE_PHPNAME, $indexType)];
            $this->building_number = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : UnitTableMap::translateFieldName('FloorNumber', TableMap::TYPE_PHPNAME, $indexType)];
            $this->floor_number = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : UnitTableMap::translateFieldName('RoomNumber', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 4; // 4 = UnitTableMap::NUM_HYDRATE_COLUMNS.
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UnitTableMap::COL_BUILDING_NUMBER)) {
            $modifiedColumns[':p' . $index++]  = 'building_number';
        }
        if ($this->isColumnModified(UnitTableMap::COL_FLOOR_NUMBER)) {
            $modifiedColumns[':p' . $index++]  = 'floor_number';
        }
        if ($this->isColumnModified(UnitTableMap::COL_ROOM_NUMBER)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'building_number':
                        $stmt->bindValue($identifier, $this->building_number, PDO::PARAM_INT);
                        break;
                    case 'floor_number':
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getBuildingNumber(),
            $keys[2] => $this->getFloorNumber(),
            $keys[3] => $this->getRoomNumber(),
        );
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) {
            $this->setBuildingNumber($arr[$keys[1]]);
        }
        if (array_key_exists($keys[2], $arr)) {
            $this->setFloorNumber($arr[$keys[2]]);
        }
        if (array_key_exists($keys[3], $arr)) {
            $this->setRoomNumber($arr[$keys[3]]);
        }
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UnitTableMap::COL_BUILDING_NUMBER)) {
            $criteria->add(UnitTableMap::COL_BUILDING_NUMBER, $this->building_number);
        }
        if ($this->isColumnModified(UnitTableMap::COL_FLOOR_NUMBER)) {
            $criteria->add(UnitTableMap::COL_FLOOR_NUMBER, $this->floor_number);
        }
        if ($this->isColumnModified(UnitTableMap::COL_ROOM_NUMBER)) {
            $criteria->add(UnitTableMap::COL_ROOM_NUMBER, $this->room_number);
        }
